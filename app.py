...
@celery.task
def delete_ami_task(instance_id, aws_access_key, aws_secret_key, aws_region):
    try:
        # Retrieve the AMI name from the database based on the instance_id
        instance = Instance.query.filter_by(instance_id=instance_id).first()
        if instance:
            ami_id = instance.ami_name
            ...
            print(f"Instance with instance_id {instance_id} not found.")
    ...
    try:
        ...
        for instance_id in instance_ids:
            ...
            if instance:
                ...
                delete_ami_task.apply_async(
                    (instance_id, customer.aws_access_key, customer.aws_secret_key, 'ap-south-1'),
                    countdown=1  # Optional: delay the task by 1 second, adjust as needed
                )
